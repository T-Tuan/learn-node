...
const servers = {
  ...
  port: '36010',
  username: 'root',
  password: 'UeAD#567!#$Gb90'
}

const localurl = `C:/caojiabin/Project/ehr-tenant-websites/dist`
const serverUrl = '/data2/solution/web'
const proName = 'ehrtenant'
...
function uploadDir(localurl, remoteurl, mren) {
  ...
  const time = ` ${new Date().getFullYear()}-${new Date().getMonth()}-${new Date().getDate()-1} ${new Date().getHours()}:${new Date().getMinutes()}:${new Date().getSeconds()}`
  connect(servers, (conn) => {
    conn.sftp(async (err, sftp) => {
      ...
      try {
        console.log('文件出现重复，正在转移目录');
        await rename(path.linuxjoin(remoteurl, mren), path.linuxjoin(remoteurl, mren) + time)
      } catch (error) {
        console.log('文件未创建');
      }
      ...
      for (const item of fileObj.fileArr) {
        ...
        console.log(`正在上传中……${(~~((nowSize*100/fileObj.allSize)*100))/100}%`);
      }
      console.log('文件上传成功！！！');
